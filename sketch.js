...
//
// This version includes a debug panel modeled after Example 2.
//
// FILE STRUCTURE:
...
function draw() {
  ...
  // Debug panel is drawn last so it stays above every screen.
...
// Matches the Example 2 debug art: dark bottom panel,
// yellow title, and small shortcut buttons.
// ------------------------------------------------------------
function drawDebugPanel() {
  ...
  let buttons = [
    ...
    { label: "2: Level 2", x: 194, w: 88 },
    { label: "3: Level 3", x: 290, w: 88 },
    { label: "W: Win", x: 386, w: 72 },
    { label: "O: Lose", x: 466, w: 78 },
  ];
  ...
  for (let i = 0; i < buttons.length; i++) {
    ...
    stroke(100, 100, 140);
    strokeWeight(1);
    rect(b.x, height - 56, b.w, 34, 4);

    fill(200);
    noStroke();
    textSize(12);
    textAlign(LEFT);
    text(b.label, b.x + 8, height - 34);
  }
}
...
function keyPressed() {
  ...
  if (key === "o" || key === "O") {
    if (!currentCard) loadLevel(currentLevel);
    clearResultState();
    gameState = STATE_OVER;
    return;
  }
...
// ------------------------------------------------------------
// clearResultState()
// Stops a pending correct/wrong timer from changing screens
// after a debug shortcut has jumped somewhere else.
// ------------------------------------------------------------
